<%@page import="com.ysc.model.Page" %>
<%@page import="com.ysc.model.Good" %>
...
<div class="details">
        ...
        <table class="table" cellspacing="0" cellpadding="0">
            ...
            <c:forEach items="${list}" var="good">
                <tr>
                    ...
                    <td><%-- <input type="checkbox" id="${good.id }" class="check" value="${good.id }" name="checkGood"/> --%><label
                            for="c1" class="label">${good.id }</label></td>
                    <td>${good.name}</td>
                    <td><c:if test="${good.featured==0 }">否</c:if><c:if test="${good.featured==1 }">是</c:if></td>
                    <td>${good.price }</td>
                    <td align="center">
                        <input type="button" value="详情" class="btn"
                               onClick="showDetail('${good.id}','${good.name }')"><c:if
                            test="${good.featured==0 }"><input type="button" value="设置" class="btn"
                                                               onClick="setFeatured('${good.id}')"></c:if><c:if
                            test="${good.featured==1 }"><input type="button" value="取消" class="btn"
                                                               onClick="cancelFeatured('${good.id}')"></c:if>
    ...
    <c:if test="${not empty page }">
        <div class='page fix'>
            ...
            <a href='GoodController/selectPageGoodForFeatured.do?currentPage=<%=myPage.getNext() %>'
               class='next'>下一页</a>
            <a href='GoodController/selectPageGoodForFeatured.do?currentPage=<%=myPage.getAllPage() %>'
               class='last'>末页</a>
